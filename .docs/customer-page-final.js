...
const Customer = () => {
    ...
    const columns = [
        {
            field: `fullName`,
            headerName: t('primaryCustomer'),
            flex: 1.2,
            cellClassName: 'name-column--cell name-column--cell--capitalize',
            renderCell: (params) => {
                // Check if this is a primary customer (has phone number and email)
                const isPrimaryCustomer = params.row.phoneNumber && params.row.phoneNumber.trim() !== '';

                return (
                    <Box>
                        <Box
                            onClick={() => isPrimaryCustomer && navigate(`/dashboard/customer/view/${params.row.phoneNumber}`)}
                            sx={{
                                cursor: isPrimaryCustomer ? 'pointer' : 'default',
                                fontWeight: 600,
                                color: isPrimaryCustomer ? '#2196f3' : '#999'
                            }}
                        >
                            {params.value}
                        </Box>
        ...
        },
        {
            ...
            flex: 0.8,
            renderCell: (params) => {
                // Only show actions for primary customers (those with phone numbers)
                const isPrimaryCustomer = params.row.phoneNumber && params.row.phoneNumber.trim() !== '';
    ...
    // Filter to show only primary customers (those who actually booked rooms)
    const primaryCustomers = useMemo(() => {
        return filteredRows.filter(customer =>
            ...
            <Container maxWidth={false} sx={{ mt: 2, px: '27px' }}>
                ...
                >
                    <Typography variant="subtitle1" sx={{ fontWeight: 600 }}>
                        {t('customerManagement')} ({primaryCustomers?.length || 0})
                    </Typography>
